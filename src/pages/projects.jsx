...
const Project = () => {
  ...
  const getImageStyles = (index) => {
    ...
    const scale = 1 - normalizedDistance * (isMobile ? 0.06 : 0.15);
    const opacity = Math.max(0.3, 1 - normalizedDistance * 0.2);
    const blur = Math.min(normalizedDistance * 2, 6);

    if (isMobile) {
      const yTranslate = distance * 30;
  ...
  return (
    ...
    >
      {/* Side Navigation */}
      <div className={`fixed ${isMobile ? 'top-[40vh] left-[3vw]' : 'left-8 top-1/2 -translate-y-1/2'} z-40`}>
        {sideNavItems.map((item, index) => (
          <motion.div
            key={index}
            className={`text-xs md:text-sm ${
              activeIndex === index ? 'text-gray-900 font-medium' : 'text-gray-600'
            } mb-2 md:mb-4 transition-colors duration-300 cursor-pointer hover:text-gray-900`}
